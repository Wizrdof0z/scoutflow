-- ============================================================================
-- CREATE CENTRAL MIDFIELDER PLAYER RATINGS TABLE
-- ============================================================================
-- This table stores pre-computed percentile-based category ratings for Central Midfielder players (CM)
-- Categories: Speed, Agility, Stamina, Pass Accuracy, Building Up, Playmaking

-- Drop existing table if exists
DROP TABLE IF EXISTS cm_player_ratings CASCADE;

-- Create the table
CREATE TABLE cm_player_ratings (
...
INSERT INTO cm_player_ratings
WITH base_data AS (
  -- Get all CM players with their metrics from source tables
  SELECT DISTINCT ON (phys.player_id, phys.competition_edition_id, phys.team_id)
    ...
    CASE 
      WHEN phys.position IN ('CM', 'LCM', 'RCM') THEN 'CM'
      ELSE phys.position
    END as position,
    
    -- Position group
    'Central Midfielder' as position_group,
  ...
  WHERE phys.position IN ('CM', 'LCM', 'RCM')
...
CREATE INDEX idx_cm_player_ratings_player_name ON cm_player_ratings(player_name);
CREATE INDEX idx_cm_player_ratings_team_name ON cm_player_ratings(team_name);
CREATE INDEX idx_cm_player_ratings_competition ON cm_player_ratings(competition_name);
CREATE INDEX idx_cm_player_ratings_player_id ON cm_player_ratings(player_id);

-- Enable Row Level Security
ALTER TABLE cm_player_ratings ENABLE ROW LEVEL SECURITY;

-- Create policy to allow public read access
CREATE POLICY "Allow public read access to cm_player_ratings" 
ON cm_player_ratings 
FOR SELECT 
TO PUBLIC 
USING (true);
