-- ============================================================================
-- CREATE WINGERS PLAYER RATINGS TABLE
-- ============================================================================
-- This table stores pre-computed percentile-based category ratings for Winger players (LW + RW)
-- Categories: Speed, Agility, Stamina, Creativity, Attacking Movement, Chance Creation, Pressure Handling

-- Drop existing table if exists
DROP TABLE IF EXISTS winger_player_ratings CASCADE;

-- Create the table
CREATE TABLE winger_player_ratings (
...
INSERT INTO winger_player_ratings
WITH base_data AS (
  -- Get all Winger players with their metrics from source tables
  SELECT DISTINCT ON (phys.player_id, phys.competition_edition_id, phys.team_id)
    ...
    -- Position group
    CASE 
      WHEN phys.position IN ('LW', 'RW') THEN 'Winger'
      ELSE 'Other'
...
CREATE INDEX idx_winger_player_ratings_player_name ON winger_player_ratings(player_name);
CREATE INDEX idx_winger_player_ratings_team_name ON winger_player_ratings(team_name);
CREATE INDEX idx_winger_player_ratings_competition ON winger_player_ratings(competition_name);
CREATE INDEX idx_winger_player_ratings_player_id ON winger_player_ratings(player_id);

-- Enable Row Level Security
ALTER TABLE winger_player_ratings ENABLE ROW LEVEL SECURITY;

-- Create policy to allow public read access
CREATE POLICY "Allow public read access to winger_player_ratings" 
ON winger_player_ratings 
FOR SELECT 
TO PUBLIC 
USING (true);
